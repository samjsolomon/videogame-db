...
-- List all employees who make over 50k alphabetically and their salaries
select name, salary
from Person, Employee
where employee.epid=person.pid
...
-- How many people does each job have?
select jobTitle, count(pid)
from Person, Employee
where employee.epid=person.pid
group by jobTitle;

-- Whats the max salary for each job?
select max(salary), jobTitle
from Person, Employee
where employee.epid=person.pid
group by jobTitle;

-- List all consoles that having more than 3 of the same color in stock
select color, count(consoleid)
from Console
group by color
having count(consoleid) > 3;

-- How many customers are in each membership?
select Membership, count(cpid)
from Customer
group by membership;

-- List all adults alphabetically
select name, timestampdiff(year,str_to_date(person.birthdate, '%m/%d/%Y'),curdate()) as age
from person
where timestampdiff(year,str_to_date(person.birthdate, '%m/%d/%Y'),curdate()) > 17
order by name;
